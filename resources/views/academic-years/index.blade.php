<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('academic-years.create') }}" class="px-4 py-2 bg-emerald-600 text-white text-sm font-semibold rounded-lg hover:bg-emerald-700 transition">
                + Tambah Tahun Ajaran
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white shadow-sm sm:rounded-lg overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            {{-- (bug fix) Gunakan variabel yang sesuai dengan controller (biasanya $academicYears) --}}
                            @forelse($academicYears as $year)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 font-medium text-gray-900">{{ $year->name }}</td>

                                    {{-- (bug fix) Tambahkan tag <td> pembuka dan atur alignment ke kanan --}}
                                    <td class="px-4 py-3 text-right">
                                        <div class="flex justify-end gap-3">
                                            <a href="{{ route('academic-years.edit', $year) }}" class="text-blue-600 hover:text-blue-800 text-xs font-semibold">Edit</a>
                            ...
                            @empty
                                {{-- (bug fix) Sesuaikan colspan dengan jumlah kolom (2) --}}
                                <tr>
                ...
                {{-- (bug fix) Gunakan variabel pagination yang benar --}}
