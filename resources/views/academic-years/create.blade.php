<x-app-layout>
    ...
    <div class="main-wrap">
        <div class="card card-form">
            ...
            <form action="{{ route('tahun-ajaran.store') }}" method="POST" onsubmit="return !this.is_active.checked || confirm('Mengaktifkan tahun ajaran baru akan menjalankan kenaikan kelas otomatis (X→XI, XI→XII, XII→lulus) dan generate saldo awal. Lanjutkan?')">
                @csrf
                ...
                <div class="field"><div class="checkbox-row"><input type="checkbox" name="is_active" value="1" class="checkbox-input" {{ old('is_active')?'checked':'' }}><span class="text-sm">Jadikan tahun ajaran ini aktif</span></div><span class="help-text">Jika diaktifkan, tahun ajaran lain akan dinonaktifkan otomatis dan sistem akan menjalankan kenaikan kelas (X→XI, XI→XII, XII→lulus) serta generate saldo awal.</span></div>
