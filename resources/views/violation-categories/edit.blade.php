<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Kategori: {{ $violationCategory->code }}</h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-lg p-6">
                ...
                @endif
                <form action="{{ route('violation-categories.update', $violationCategory) }}" method="POST" class="space-y-4">
                    @csrf @method('PUT')
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Kode</label>
                        <input type="text" name="code" value="{{ old('code', $violationCategory->code) }}" required maxlength="20" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-tertiary focus:border-tertiary">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Nama</label>
                        <input type="text" name="name" value="{{ old('name', $violationCategory->name) }}" required maxlength="150" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-tertiary focus:border-tertiary">
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="points" value="{{ old('points', $violationCategory->points) }}" required min="1" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-tertiary focus:border-tertiary">
                        </div>
                    </div>
                    <div>
